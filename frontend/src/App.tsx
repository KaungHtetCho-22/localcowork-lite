...
// ── Tauri startup helper ──────────────────────────────────────────────────────

async function waitFor(fn: () => Promise<unknown>, timeout: number): Promise<void> {
  const start = Date.now();
  while (Date.now() - start < timeout) {
    try { await fn(); return; } catch { /* not ready yet */ }
      ...
      <div className={`max-w-[80%] ${isUser ? "order-2" : "order-1"}`}>
        ...
        >
          <ReactMarkdown
            components={{
              a: ({ href, children }) => (
                <a href={href} className="text-[#1a8cff] underline hover:text-[#6ee7b7] transition-colors"
                  target="_blank" rel="noopener noreferrer">{children}</a>
              ),
              table: ({ ...props }) => <table className="text-xs border-collapse w-full my-2" {...props} />,
              th:    ({ ...props }) => <th className="border border-white/20 px-2 py-1 text-left text-slate-300 bg-white/5" {...props} />,
              td:    ({ ...props }) => <td className="border border-white/20 px-2 py-1 text-slate-400" {...props} />,
              ...
              code:  ({ ...props }) => <code className="bg-black/40 px-1 rounded text-[#6ee7b7] text-xs" {...props} />,
...
export default function App() {
  ...
  // ── Tauri startup state (only used when running as desktop app) ───────────
  const [ready, setReady]           = useState(!IS_TAURI); // skip loading if in browser
  const [loadingMsg, setLoadingMsg] = useState("Starting model server...");

  const wsRef     = useRef<WebSocket | null>(null);
  const bottomRef = useRef<HTMLDivElement>(null);
  const inputRef  = useRef<HTMLTextAreaElement>(null);

  // ── Tauri sidecar readiness check ────────────────────────────────────────
  useEffect(() => {
    if (!IS_TAURI) return; // browser mode — skip entirely

    const poll = async () => {
      try {
        const { invoke } = await import("@tauri-apps/api/core");

        setLoadingMsg("Waiting for model server...");
        await waitFor(() => invoke("get_model_status"), 300_000) // 5 min;
        
        setLoadingMsg("Starting backend...");
        await waitFor(() => invoke("get_backend_status"), 30_000);

        setReady(true);
      } catch {
        setLoadingMsg("Failed to start. Check logs.");
      }
    };
    ...
    ws.onmessage = (e) => {
      ...
      setMessages(prev => {
        ...
        if (event.type === "tool_call") {
          updated.traces = [
            ...updated.traces,
            { tool: event.tool, server: parseServer(event.tool), arguments: event.arguments, pending: true },
          ];
  ...
  useEffect(() => {
    if (!ready) return; // don't connect until sidecars are up (Tauri mode)
  ...
  const handleConfirm = (approved: boolean) => {
    ...
    wsRef.current?.send(JSON.stringify({ type: "confirm", tool_call_id: pendingConfirm.tool_call_id, approved }));
  ...
  if (!ready) {
    return (
      <div className="h-screen bg-[#010409] flex flex-col items-center justify-center gap-4">
        <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-[#1a8cff]/20 to-[#6ee7b7]/20
                        flex items-center justify-center border border-white/10">
          <Activity size={18} className="text-[#6ee7b7] animate-pulse" />
        </div>
        <p className="text-sm text-slate-400 font-mono">{loadingMsg}</p>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-6 text-center">
            <div>
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#1a8cff]/20 to-[#6ee7b7]/20
                              flex items-center justify-center mx-auto mb-4 border border-white/10">
            ...
            <div className="grid grid-cols-2 gap-2 max-w-lg w-full">
              {SUGGESTIONS.map(s => (
                <button key={s.prompt} onClick={() => send(s.prompt)}
                  className="text-left text-xs text-slate-400 bg-[#0d1117] border border-white/5
                             rounded-xl px-3 py-2.5 hover:border-[#1a8cff]/40 hover:text-slate-200 transition-all">
                  {s.label}
                </button>
              ))}
            </div>
      ...
      <div className="px-6 py-4 border-t border-white/5 bg-[#0d1117]">
        <div className="flex gap-3 items-end max-w-4xl mx-auto">
          <textarea
            ...
            className="flex-1 bg-[#161b22] border border-white/5 rounded-xl px-4 py-3 text-sm
                       text-slate-200 placeholder-slate-600 focus:outline-none focus:border-[#1a8cff]/50
                       resize-none disabled:opacity-40 transition-colors"
            style={{ minHeight: "44px", maxHeight: "120px" }}
          />
          <button onClick={() => send(input)} disabled={!connected || busy || !input.trim()}
            className="w-11 h-11 rounded-xl bg-[#1a8cff] flex items-center justify-center
                       hover:bg-[#1a8cff]/80 disabled:opacity-30 disabled:cursor-not-allowed transition-all shrink-0">
            {busy ? <Loader2 size={16} className="animate-spin" /> : <Zap size={16} />}
          </button>
          <button title="Reset session"
            onClick={() => {
              setMessages([]);
